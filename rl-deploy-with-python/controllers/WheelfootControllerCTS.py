...
class WheelfootController:
    def __init__(self, model_dir, robot, robot_type, start_controller):
        ...
        self.model_student = f'{model_dir}/{self.robot_type}/policy/gait_generator.onnx'
    ...
    def initialize_onnx_models(self):
        ...
        self.student_session = ort.InferenceSession(self.model_student, sess_options=session_options, providers=cpu_providers)
        self.student_input_names = [self.student_session.get_inputs()[i].name for i in range(self.student_session.get_inputs().__len__())]
        self.student_output_names = [self.student_session.get_outputs()[i].name for i in range(self.student_session.get_outputs().__len__())]
        self.student_input_shapes = [self.student_session.get_inputs()[i].shape for i in range(self.student_session.get_inputs().__len__())]
        self.student_output_shapes = [self.student_session.get_outputs()[i].shape for i in range(self.student_session.get_outputs().__len__())]

        # 2. 分析ONNX模型
        print("\n2. Policy模型分析:")
        policy_input = self.policy_session.get_inputs()[0]
        policy_output = self.policy_session.get_outputs()[0]
        print(f"Policy输入: {policy_input.name}, 形状: {policy_input.shape}")
        print(f"Policy输出: {policy_output.name}, 形状: {policy_output.shape}")
        
        print("\n3. Encoder模型分析:")
        encoder_input = self.encoder_session.get_inputs()[0]
        encoder_output = self.encoder_session.get_outputs()[0]
        print(f"Encoder输入: {encoder_input.name}, 形状: {encoder_input.shape}")
        print(f"Encoder输出: {encoder_output.name}, 形状: {encoder_output.shape}")

        print("\n4. Student模型分析:")
        student_input = self.student_session.get_inputs()[0]
        student_output = self.student_session.get_outputs()[0]
        print(f"Student输入: {student_input.name}, 形状: {student_input.shape}")
        print(f"Student输出: {student_output.name}, 形状: {student_output.shape}")
    ...
    def load_config(self, config_file):
        ...
        self.student_input_size = self.encoder_input_size + self.commands_size
        ...
        print(f"关节位置索引: {self.joint_pos_idxs}")
    ...
    def align_robot_state(self, robot_state: datatypes.RobotState):
        ...
        aligned_robot_state.q[1] = robot_state.q[4]
        aligned_robot_state.dq[1] = robot_state.dq[4]
        aligned_robot_state.tau[1] = robot_state.tau[4]
        ...
        aligned_robot_state.q[3] = robot_state.q[5]
        aligned_robot_state.dq[3] = robot_state.dq[5]
        aligned_robot_state.tau[3] = robot_state.tau[5]
        ...
        aligned_robot_state.q[5] = robot_state.q[6]
        aligned_robot_state.dq[5] = robot_state.dq[6]
        aligned_robot_state.tau[5] = robot_state.tau[6]

        aligned_robot_state.q[6] = robot_state.q[3]
        aligned_robot_state.dq[6] = robot_state.dq[3]
        aligned_robot_state.tau[6] = robot_state.tau[3]
        ...
        if self.stand_percent < 1:
            for j in range(len(self.joint_names)):
                if j< 6:
                    ...
                    self.set_joint_command_aligned(j, pos_des, 0, 0, self.control_cfg['stiffness'], self.control_cfg['damping'])
                else:
                    self.set_joint_command_aligned(0, 0, 0, self.wheel_joint_damping, 0, 0)
    ...
    def handle_walk_mode(self):
        self.init_state["hip_L_Joint"] = -0.0;
        self.init_state["hip_R_Joint"] = 0.0;
        ...
        for i in range(len(joint_pos)):
            if i < 6:  # For the first six joints (legs)
                ...
                self.set_joint_command_aligned(i, pos_des, 0, 0, self.control_cfg['stiffness'], self.control_cfg['damping'])
                ...
                self.set_joint_command_aligned(i, 0, velocity_des, 0, 0, self.wheel_joint_damping)
    ...
    def compute_observation(self):
        ...
        print(f"当前观察维度: {obs.shape}, 期望维度: {self.observations_size}")
    ...
    def compute_actions(self):
        ...
        input_tensor = np.concatenate([self.encoder_out, self.observations, self.scaled_commands, self.student_out], axis=0)
        input_tensor = input_tensor.astype(np.float32)

        print(f"当前动作输入维度: {input_tensor.shape}, 期望维度: {self.policy_input_shapes[0]}")
        ...
        print(f"当前动作输出维度: {output[0].shape}, 期望维度: {self.policy_output_shapes[0]}")
        
        # Flatten the output and store it as actions
        self.actions = np.array(output).flatten()
        print(f"当前动作输出: {self.actions}")
    ...
    def compute_student(self):
        """
        Computes the student output based on the encoder output and current commands.

        This method first concatenates the encoder output and commands into a single input tensor.
        Then it converts the input tensor to the float32 data type. After that, it creates a dictionary
        of inputs for the student session and runs the student session to get the output. Finally,
        it flattens the output and stores it as the actions.
        """
        # Concatenate the encoder output and commands into a single tensor and convert to float32
        input_tensor = np.concatenate([self.proprio_history_buffer, self.commands], axis=0)
        input_tensor = input_tensor.astype(np.float32)

        # Create a dictionary of inputs for the student session
        inputs = {self.student_input_names[0]: input_tensor}

        # Run the student session and get the output
        output = self.student_session.run(self.student_output_names, inputs)

        # Flatten the output and store it as actions
        self.student_out = np.array(output).flatten()
    ...
    def set_joint_command_aligned(self, joint_index, q, dq, tau, kp, kd):
        
        if joint_index == 2:
            self.robot_cmd.q[1] = q
            self.robot_cmd.dq[1] = dq
            self.robot_cmd.tau[1] = tau
            self.robot_cmd.Kp[1] = kp
            self.robot_cmd.Kd[1] = kd        
        elif joint_index == 4:
            self.robot_cmd.q[2] = q
            self.robot_cmd.dq[2] = dq
            self.robot_cmd.tau[2] = tau
            self.robot_cmd.Kp[2] = kp
            self.robot_cmd.Kd[2] = kd
        elif joint_index == 6:
            self.robot_cmd.q[3] = q
            self.robot_cmd.dq[3] = dq
            self.robot_cmd.tau[3] = tau
            self.robot_cmd.Kp[3] = kp
            self.robot_cmd.Kd[3] = kd
        elif joint_index == 1:
            self.robot_cmd.q[4] = q
            self.robot_cmd.dq[4] = dq
            self.robot_cmd.tau[4] = tau
            self.robot_cmd.Kp[4] = kp
            self.robot_cmd.Kd[4] = kd
        elif joint_index == 3:
            self.robot_cmd.q[5] = q
            self.robot_cmd.dq[5] = dq
            self.robot_cmd.tau[5] = tau
            self.robot_cmd.Kp[5] = kp
            self.robot_cmd.Kd[5] = kd
        elif joint_index == 5:
            self.robot_cmd.q[6] = q
            self.robot_cmd.dq[6] = dq
            self.robot_cmd.tau[6] = tau
            self.robot_cmd.Kp[6] = kp
            self.robot_cmd.Kd[6] = kd
        else:
            self.robot_cmd.q[joint_index] = q
            self.robot_cmd.dq[joint_index] = dq
            self.robot_cmd.tau[joint_index] = tau
